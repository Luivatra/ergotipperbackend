...
class TransactionService(val db : TransactionRepository,
    ...
    fun findTransactions(fromWallet: String, fromAccount: String): List<Transaction> = db.findTransactions(fromWallet,fromAccount)
    ...
    fun post(transaction: Transaction) : SendTransactionResult {
        ...
        if (transaction.token.isNotEmpty()) {
            ...
            if(token != null)
            {
                tokensToSend = listOf<ErgoToken>(ErgoToken(token.token_id,
                    floor(transaction.token_amount*10.0.pow(token.decimals.toDouble())).toLong()
                ))
            }
            else
            {
                return SendTransactionResult(false,errorMsg = "Unknown token name: " + transaction.token)
            }
        }
        val tx = ErgoUtil.sendTx(walletService.getSeed(transaction.from_wallet),walletService.getAddress(transaction.to_wallet),transaction.amount, tokensToSend)
        if(tx.success)
    ...
    {
        val fromAccount = accountService.findAccount(transactionLike.fromPlatform,transactionLike.fromUsername)
        if (fromAccount != null) {
            val toAccount = accountService.findAccount(transactionLike.toPlatform, transactionLike.toUsername)
            if (toAccount != null) {
                return post(
                    Transaction(
